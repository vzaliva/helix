...
** Automation for proofs like ~DynWin_pure~.
   ...
   One way around this is to follow MSHCOL structure. Another solution, is
   to build proof as we compile MSHCOL via template-coq.

   Finally, it /might/ be possible to follow the structure of DSHCOL
   if the mapping of MSHCOL to DSHCOL is injective. Here we will take
   advantage of LTAC patter matching which provides us with look-ahead
   parser to match DHSCOL fragments to MSHCOL operators.

   
* Unfinished
** Division by zero in DSHCOL
   [[file:../coq/DSigmaHCOL/DSigmaHCOL.v::33][NExpr]] ([[file:../coq/DSigmaHCOL/DSigmaHCOLEval.v::61][evalNexpr]])
...
** StructTactics dependency
   Instead of included copy of [[file:~/src/helix/coq/Tactics/StructTactics.v][StructTactics.v]] we can pin an opam
   package for git repo:

   =opam pin add coq-struct-tact https://github.com/uwplse/StructTact.git -k git=

   Related [[https://github.com/uwplse/StructTact/issues/55][ticket]].
