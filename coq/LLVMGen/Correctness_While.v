...
Proof.
  intros * IN PREF UNIQUE_IDENTS NEXTBLOCK_ID * GEN A LVAR_FRESH *.
  ...
  - (* Inductive case *)
    ...
    (* Step 4 : Back to starting from loopcontblock and have reestablished everything at the next index:
        conclude by IH *)
    intros ? (? & ? & ? & ?) (LOOPVAR' & [? ->] & (IH' & LOC)).
    eapply eqit_mon; auto.
    2 : apply IH; try lia; try split; auto.
    intros B (? & ? & ? & ?) (? & INV' & LOC').
    split; try split; auto.
    Notation "k '!->' v ';' l" := (alist_add k v l)
                                    (at level 100, v at next level, right associativity).
    cbn.
    (* try to relate [li] and [l] somehow. *)
...
Proof.
  ...
  (*   apply free_in_convert_typ with (env := []) in EXIT; cbn in EXIT; rewrite ?convert_typ_ocfg_app in EXIT. *)
  (*   apply wf_ocfg_bid_convert_typ with (env := []) in UNIQUE; cbn in UNIQUE; rewrite ?convert_typ_ocfg_app in UNIQUE. *)
  (*   cbn; rewrite ?convert_typ_ocfg_app. *)
  (*   cbn in GEN_IND; rewrite ?convert_typ_ocfg_app in GEN_IND. *)
...
Qed.
 *)
(* END work in progress to add the transport of [] as a postcondition through the loop *)
