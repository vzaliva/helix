...
Module MDSigmaHCOLEval (Import CT : CType) (Import ESig:MDSigmaHCOLEvalSig CT).
  ...
  Definition Event := MemEvent +' FailE.

  Definition tfail {A: Type} (msg: string): itree Event A :=
  ...
  Definition lift_err {A} {E} `{FailE -< E} (m:err A) : itree E A :=
    match m with
    | inl x => throw x
    | inr x => ret x
    end.
  ...
  Fixpoint evalAexp (σ: evalContext) (e:AExpr): itree Event t :=
    match e with
    | AVar i =>
      v <- lift_err (context_lookup "AVar not found" σ i);;
        (match v with
         | DSHCTypeVal x => ret x
         | _ => tfail "invalid AVar type"
         end)
    | AConst x => ret x
