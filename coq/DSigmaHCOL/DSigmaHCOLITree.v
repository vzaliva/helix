...
Ltac state_step :=
  ...
  | |- context[interp_state _ (trigger _) _] => unfold trigger, Trigger_ITree; rewrite interp_state_trigger
  | |- context[interp_state _ (ITree.trigger _) _] => rewrite interp_state_trigger
...
Module MDSigmaHCOLITree (Import CT : CType) (Import ESig:MDSigmaHCOLEvalSig CT).
  ...
  Notation DSigEvent := (MemEvent +' StaticFailE +' DynamicFailE).

  Section WithContext.

    Context {Event C1 C2 C3: Type -> Type}.
    Context `{MemEvent +? C1 -< Event}.
    Context `{StaticFailE +? C2 -< Event}.
    Context `{DynamicFailE +? C3 -< Event}.

    Definition Sfail {A: Type} {E C} `{DynamicFailE +? C -< E} (msg: string): itree E A :=
      vis (Throw msg) (fun (x: void) => match x with end).

    Definition Dfail {A: Type} {E C} `{DynamicFailE +? C -< E} (msg: string): itree E A :=
      vis (Throw msg) (fun (x: void) => match x with end).

    Definition lift_Serr {A} {E C} `{StaticFailE +? C -< E} (m:err A) : itree E A :=
      match m with
      | inl x => throw x
      | inr x => ret x
      end.

    Definition lift_Derr {A} {E C} `{DynamicFailE +? C -< E} (m:err A) : itree E A :=
      match m with
      | inl x => throw x
      | inr x => ret x
      end.
    ...
    Definition Mem_handler {E C} `{DynamicFailE +? C -< E}: MemEvent ~> Monads.stateT memory (itree E) :=
      fun T e mem =>
        match e with
    ...
    Definition interp_Mem {E C F} `{DynamicFailE +? C -< E} `{MemEvent +? E -< F}:
      itree F ~> Monads.stateT memory (itree E) :=
      interp_state (over Mem_handler).
    Arguments interp_Mem {E C F _ _ T} _ _.
  ...
  Section Eval_Denote_Equiv.
    ...
    Ltac inv_option :=
      ...
      Context {Event C1 C2 C3: Type -> Type}.
      Context `{MemEvent +? C1 -< Event}.
      Context `{StaticFailE +? C2 -< C1}.
      Context `{DynamicFailE +? C3 -< C2}.
      Arguments interp_Mem {E C F _ _} [T].
      ...
      Proof.
        ...
        - inv_eval; state_steps; eexists; left; state_steps.
          admit. (* To be fixed *)
          admit. (* To be fixed *)
        - inl_inr.
      Admitted.
